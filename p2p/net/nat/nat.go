...
func (nat *NAT) rediscoverNAT() {
	...
	if err != nil {
		log.Warnf("NAT rediscovery failed: %v", err)
		nat.natmu.Lock()
		defer nat.natmu.Unlock()
	...
	for e := range nat.mappings {
		...
		nat.mappings[e] = extPort
		if extPort != 0 {
			log.Infof("NAT mapping restored after rediscovery: %s %d -> %d", e.protocol, e.port, extPort)
		}
	}
	nat.mappingmu.Unlock()
...
func getExternalAddress(natInstance nat.NAT) netip.Addr {
	...
	if !ok {
		log.Debugf("Failed to parse external address %v", extIP)
		return netip.Addr{}
	}
	return extAddr
}
