...
type addressManagerArgs struct {
...
type addressManagerTestCast struct {
	*addressManager
	PushRelay        func(relayAddrs []ma.Multiaddr)
	PushReachability func(rch network.Reachability)
}

func newAddressManagerTestCast(t *testing.T, args addressManagerArgs) addressManagerTestCast {
	eb := eventbus.NewBus()
	...
	rAddrEM, err := eb.Emitter(new(event.EvtAutoRelayAddrsUpdated), eventbus.Stateful)
	require.NoError(t, err)

	rchEM, err := eb.Emitter(new(event.EvtLocalReachabilityChanged), eventbus.Stateful)
	require.NoError(t, err)

	return addressManagerTestCast{
		addressManager: am,
		PushRelay: func(relayAddrs []ma.Multiaddr) {
...
func TestAddressManager(t *testing.T) {
	...
	t.Run("NAT Address", func(t *testing.T) {
		am := newAddressManagerTestCast(t, addressManagerArgs{
			NATManager: &mockNatManager{
				HasDiscoveredNATFunc: func() bool { return true },
				GetMappingFunc: func(addr ma.Multiaddr) ma.Multiaddr {
		...
		am.signalAddressChange()
		require.EventuallyWithT(t, func(collect *assert.CollectT) {
			assert.Contains(collect, am.Addrs(), ma.StringCast("/ip4/1.2.3.4/udp/1/quic-v1"), "%s\n%s", am.Addrs(), ma.StringCast("/ip4/1.2.3.4/udp/1/quic-v1"))
	...
	t.Run("NAT And Observed Address", func(t *testing.T) {
		...
		require.EventuallyWithT(t, func(collect *assert.CollectT) {
			assert.Contains(collect, am.Addrs(), ma.StringCast("/ip4/1.2.3.4/udp/1/quic-v1"), "%s\n%s", am.Addrs(), ma.StringCast("/ip4/1.2.3.4/udp/1/quic-v1"))
			assert.Contains(collect, am.Addrs(), ma.StringCast("/ip4/2.2.2.2/tcp/1"), "%s\n%s", am.Addrs(), ma.StringCast("/ip4/2.2.2.2/tcp/1"))
		}, 5*time.Second, 30*time.Millisecond)
	})
	t.Run("Only Observed Address", func(t *testing.T) {
		...
		am.signalAddressChange()
		require.EventuallyWithT(t, func(collect *assert.CollectT) {
			assert.NotContains(collect, am.Addrs(), ma.StringCast("/ip4/2.2.2.2/tcp/1"), "%s\n%s", am.Addrs(), ma.StringCast("/ip4/2.2.2.2/tcp/1"))
			assert.Contains(collect, am.Addrs(), ma.StringCast("/ip4/3.3.3.3/udp/1/quic-v1"), "%s\n%s", am.Addrs(), ma.StringCast("/ip4/3.3.3.3/udp/1/quic-v1"))
		}, 5*time.Second, 30*time.Millisecond)
	})
	t.Run("Public Addrs Removed When Private", func(t *testing.T) {
		...
		require.EventuallyWithT(t, func(collect *assert.CollectT) {
			assert.NotContains(collect, am.Addrs(), ma.StringCast("/ip4/3.3.3.3/udp/1/quic-v1"), "%s\n%s", am.Addrs(), ma.StringCast("/ip4/3.3.3.3/udp/1/quic-v1"))
			assert.Contains(collect, am.Addrs(), relayAddr, "%s\n%s", am.Addrs(), relayAddr)
			assert.Contains(collect, am.AllAddrs(), ma.StringCast("/ip4/3.3.3.3/udp/1/quic-v1"), "%s\n%s", am.AllAddrs(), ma.StringCast("/ip4/3.3.3.3/udp/1/quic-v1"))
		}, 5*time.Second, 30*time.Millisecond)
	})
	...
	t.Run("AddressFactory gets relay addresses", func(t *testing.T) {
		...
		require.EventuallyWithT(t, func(collect *assert.CollectT) {
			assert.Contains(collect, am.Addrs(), ma.StringCast("/ip4/3.3.3.3/udp/1/quic-v1"), "%s\n%s", am.Addrs(), ma.StringCast("/ip4/3.3.3.3/udp/1/quic-v1"))
			assert.NotContains(collect, am.Addrs(), relayAddr, "%s\n%s", am.Addrs(), relayAddr)
		}, 5*time.Second, 30*time.Millisecond)
	})
	...
	t.Run("updates addresses on signaling", func(t *testing.T) {
		...
		require.Contains(t, am.Addrs(), a1)
		require.NotContains(t, am.Addrs(), a2)
		close(updateChan)
		am.signalAddressChange()
		require.EventuallyWithT(t, func(collect *assert.CollectT) {
			assert.Contains(collect, am.Addrs(), a2)
			assert.NotContains(collect, am.Addrs(), a1)
		}, 5*time.Second, 30*time.Millisecond)
	})
}
