# test-plans test implementation (OUTDATED)

This file is kept for historical reference.

The `libp2p/test-plans` repository has been refactored since this was written (no `transport-interop/`, no `npm run test` at the suite root).
