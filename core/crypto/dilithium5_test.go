...
func TestDilithium5KeyGeneration(t *testing.T) {
	priv, pub, err := GenerateDilithium5Key(rand.Reader)
	require.NoError(t, err)
	require.NotNil(t, priv)
	require.NotNil(t, pub)

	// Test key types
	assert.Equal(t, pb.KeyType_Dilithium5.Number(), priv.Type().Number())
	assert.Equal(t, pb.KeyType_Dilithium5.Number(), pub.Type().Number())
...
func TestDilithium5Signing(t *testing.T) {
	priv, pub, err := GenerateDilithium5Key(rand.Reader)
	require.NoError(t, err)

	message := []byte("Hello, Dilithium5 world!")
...
func TestDilithium5KeyMarshaling(t *testing.T) {
	priv, pub, err := GenerateDilithium5Key(rand.Reader)
	require.NoError(t, err)
	...
	// Test unmarshaling
	unmarshaledPriv, err := UnmarshalDilithium5PrivateKey(privBytes)
	require.NoError(t, err)
	assert.True(t, priv.Equals(unmarshaledPriv))

	unmarshaledPub, err := UnmarshalDilithium5PublicKey(pubBytes)
	require.NoError(t, err)
