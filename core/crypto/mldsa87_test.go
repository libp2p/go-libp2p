...
func TestMLDSA87KeyGeneration(t *testing.T) {
	priv, pub, err := GenerateMLDSA87Key(rand.Reader)
	require.NoError(t, err)
	require.NotNil(t, priv)
	require.NotNil(t, pub)

	// Test key types
	assert.Equal(t, pb.KeyType_MLDSA87.Number(), priv.Type().Number())
	assert.Equal(t, pb.KeyType_MLDSA87.Number(), pub.Type().Number())
...
func TestMLDSA87Signing(t *testing.T) {
	priv, pub, err := GenerateMLDSA87Key(rand.Reader)
	require.NoError(t, err)

	message := []byte("Hello, ML-DSA-87 world!")
...
func TestMLDSA87KeyMarshaling(t *testing.T) {
	priv, pub, err := GenerateMLDSA87Key(rand.Reader)
	require.NoError(t, err)
	...
	// Test unmarshaling
	unmarshaledPriv, err := UnmarshalMLDSA87PrivateKey(privBytes)
	require.NoError(t, err)
	assert.True(t, priv.Equals(unmarshaledPriv))

	unmarshaledPub, err := UnmarshalMLDSA87PublicKey(pubBytes)
	require.NoError(t, err)
