...
func TestDilithium3KeyGeneration(t *testing.T) {
	priv, pub, err := GenerateDilithium3Key(rand.Reader)
	require.NoError(t, err)
	require.NotNil(t, priv)
	require.NotNil(t, pub)

	// Test key types
	assert.Equal(t, pb.KeyType_Dilithium3.Number(), priv.Type().Number())
	assert.Equal(t, pb.KeyType_Dilithium3.Number(), pub.Type().Number())
...
func TestDilithium3Signing(t *testing.T) {
	priv, pub, err := GenerateDilithium3Key(rand.Reader)
	require.NoError(t, err)

	message := []byte("Hello, Dilithium3 world!")
...
func TestDilithium3KeyMarshaling(t *testing.T) {
	priv, pub, err := GenerateDilithium3Key(rand.Reader)
	require.NoError(t, err)
	...
	// Test unmarshaling
	unmarshaledPriv, err := UnmarshalDilithium3PrivateKey(privBytes)
	require.NoError(t, err)
	assert.True(t, priv.Equals(unmarshaledPriv))

	unmarshaledPub, err := UnmarshalDilithium3PublicKey(pubBytes)
	require.NoError(t, err)
