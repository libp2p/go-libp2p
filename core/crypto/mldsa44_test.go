...
func TestMLDSA44KeyGeneration(t *testing.T) {
	priv, pub, err := GenerateMLDSA44Key(rand.Reader)
	require.NoError(t, err)
	require.NotNil(t, priv)
	require.NotNil(t, pub)

	// Test key types
	assert.Equal(t, pb.KeyType_MLDSA44.Number(), priv.Type().Number())
	assert.Equal(t, pb.KeyType_MLDSA44.Number(), pub.Type().Number())
...
func TestMLDSA44Signing(t *testing.T) {
	priv, pub, err := GenerateMLDSA44Key(rand.Reader)
	require.NoError(t, err)

	message := []byte("Hello, ML-DSA-44 world!")
...
func TestMLDSA44KeyMarshaling(t *testing.T) {
	priv, pub, err := GenerateMLDSA44Key(rand.Reader)
	require.NoError(t, err)
	...
	// Test unmarshaling
	unmarshaledPriv, err := UnmarshalMLDSA44PrivateKey(privBytes)
	require.NoError(t, err)
	assert.True(t, priv.Equals(unmarshaledPriv))

	unmarshaledPub, err := UnmarshalMLDSA44PublicKey(pubBytes)
	require.NoError(t, err)
